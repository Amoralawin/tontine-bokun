    ...
    <div className="relative inline-block text-left">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute left-0 mt-2 w-64 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl z-50 py-2 overflow-hidden">
            ...
            <div className="max-h-72 overflow-y-auto py-1">
              <button
                ...
                className={`w-full text-left px-3.5 py-2.5 flex items-center justify-between hover:bg-emerald-500/10 dark:hover:bg-emerald-400/10 border-b border-slate-100 dark:border-slate-800/50 transition-colors ${
                  isAutoDetected ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 font-semibold" : "text-slate-700 dark:text-slate-300"
                }`}
              >
                <div className="flex items-center gap-2.5">
                  <span className="text-lg">🤖</span>
                  <div>
                    <div className="text-xs font-semibold">Auto-détection (Région)</div>
                    <div className="text-[10px] text-slate-400 dark:text-slate-500">
                      {isAutoDetected && detectedRegionName ? `Actif : ${detectedRegionName}` : "Détecter selon ma position"}
                    </div>
                  </div>
                </div>
                {isAutoDetected && <Check className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />}
              </button>
